                ...
                <div class="card-header">
                    <div class="row align-items-center">
                        ...
                        <div class="col-md-6 text-end">
                            @can('volume-journal.create')
                                <a href="{{ route('admin.volume-journal.create') }}" class="btn btn-primary">
                ...
                <div class="card-body">
                    <div id="buttons-datatables_wrapper" class="dataTables_wrapper dt-bootstrap5 no-footer">
                        <table id="buttons-datatables" class="display table table-bordered dataTable no-footer"
                            ...
                            <tbody>
                            @forelse ($journal_volumes as $data)
                                <tr>
                                    <td>{{ $loop->iteration }}</td>

                                    <td>{{ Str::limit( $data->title,60)  }}</td>
                                    <td>{{ $data->writer_name }}</td>
                                    <td>{{ $data->journal->title??null }}</td>
                                    <td>{{ $data->volume->title??null }}</td>
                                    <td>{{ $data->issue->title??null }}</td>
                                    <td>
                                        @if($data->deleted_at)
                                            ...
                                            <ul class="dropdown-menu dropdown-menu-end" style="">
                                                    ...
                                                    @can('volume-journal.destroy')
                                                        <li>
                                                            ...
                                                            <form id="delete-form-{{ $data->id }}"
                                                                  action="{{ route('admin.volume-journal.destroy', $data->id) }}"
                                                        ...
                                                        <li>
                                                            <a class="dropdown-item edit-item-btn"
                                                               href="{{ route('admin.volume-journal.restore', $data->id) }}">
                                                ...
                                                @else
                                                    @can('volume-journal.index')
                                                        <li>
                                                            <a class="dropdown-item edit-item-btn"
                                                               href="{{ route('admin.volume-journal.show', $data->id) }}">
                                                                <i class="bx bxs-show align-bottom me-2 text-muted"></i>
                                                                Show
                                                            </a>
                                                        </li>
                                                    @endcan
                                                    @can('volume-journal.edit')
                                                        <li>
                                                            <a class="dropdown-item edit-item-btn"
                                                               href="{{ route('admin.volume-journal.edit', $data->id) }}">
                                                    ...
                                                    @can('volume-journal.destroy')
                                                        <li>
                                                            ...
                                                            <form id="trash-form-{{ $data->id }}"
                                                                  action="{{ route('admin.volume-journal.trash', $data->id) }}"
                                                                  method="POST" style="display: none;">
                                                                @csrf
                                                            </form>
                                                        </li>
